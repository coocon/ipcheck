...
define(function (require) {
     ...
     * @Param {string} item 查看是否存在的值
     * @return {boolean} 
     */
    var contains = function (arr, item) {
        $.inArray(item, arr) >=0 ? true: false;
    };
    ...
    var requestQueue = (function () {
        ...
        return {
            ...
            reduce: function (tokenId) {
                ...
                var index = T.inArray(tokenId, queue);
                if (index != -1) {
                    queue.splice(queue, 1);
                }
                if (queue.length <= 0) {
                    layer.tipHide();
                }
    ...
    */
    ...
    errorHandlers.def = function (status, obj) {
        var msg = (obj && obj.statusInfo) || '系统开小差了，请稍后重试...';
    ...
    * seesion失效
    */
    errorHandlers['401'] = function (status, obj) {
        var msg = obj.statusInfo || '当前会话失效，请重新登录';

        layer.warning(msg, function () {
            if (window.parent) {
                window.parent.location = obj.data.location;
                }
            else {
     ...
     * 系统错误操作
     */
    errorHandlers['404'] = function (status, obj) {
        var msg =  '服务异常，请稍后再试...';
     ...
     * 系统错误操作
     */
    errorHandlers['501'] = function (status, obj) {
        var msg = obj.statusInfo || '系统错误，请联系管理员';
     ...
     * 繁忙操作
     */
    errorHandlers['502'] = function (status, obj) {
        var msg = obj.statusInfo || '系统正忙，请稍后再试';
    ...
    */
    function creatSuccessHandler(options) {
        ...
        var callback = function(data, obj) {
            ...
            //调用回调函数如果不是false,这里统一增加跳转的功能
            var result = success.call(null, data, obj);
            //默认
            if (result !== false) {
                defSuccess.call(null, data, obj);
            }
        };
         ...
         * 默认成功的处理，包括跳转
         */
        function defSuccess(data, obj) {
        
            if (obj && obj.statusInfo 
                    && data && data.location
            ) {
                layer.alert(obj.statusInfo, function () { 
    ...
    */
     ...
     * 针对query字符串的处理
     */
    function addParams4Query(query, params) {
        return query + (query.length >= 0 ? '&' : '') + params;
    }
    ...
    */
    function request(url, options) {
        ...
        //创建 成功和失败的回调
        options.success = creatSuccessHandler(options);
        options.error = creatFailureHandler(options);
    ...
    return {
         ...
         */
        get: function (url, data, onsuccess, onfailure, options) {
            var _options = {
                method: 'get',
                data: data,
                onsuccess: onsuccess,
